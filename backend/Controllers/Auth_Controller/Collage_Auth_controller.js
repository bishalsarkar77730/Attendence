...
const validateUsername = async (username) => {
  const user = await CollageUser.findOne({ username });
  return user ? false : true;
};

const validateNumber = async (number) => {
  const user = await CollageUser.findOne({ number });
  return user ? false : true;
};
...
export const signup = async (req, res, next) => {
  try {
    ...
    const newUser = new CollageUser({ ...newdata, password: hash });
    await newUser.save();
...
export const signin = async (req, res, next) => {
  try {
    ...
    const user = await CollageUser.findOne({ username }).select("+password");
    if (!user) {
